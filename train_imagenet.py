...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.supervised:     
        ...
        ds_train = DermaMNIST(split='train', transform=AUG_s, download=True, root='./data/')
        ds_val = DermaMNIST(split='val', transform=transforms.ToTensor(), download=True, root='./data/')
        ds_test = DermaMNIST(split='test', transform=transforms.ToTensor(), download=True, root='./data/')
        ...
        model = BYOL(model, image_size=28, hidden_layer=-2, augment_fn=AUG_s)

        ds_train = medmnist.dataset.__dict__[args.data](split='train', transform=transforms.ToTensor(), download=True, root='./data/')
        ds_val = medmnist.dataset.__dict__[args.data](split='val', transform=transforms.ToTensor(), download=True, root='./data/')
        # if args.distributed:
    ...
    if args.test:
        loss_test, auc_test, acc_test = validate(loader_test, model.net, criterion, args)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        args.loss_train.append(train(loader_train, model, criterion, optimizer, scheduler, args))

        loss_val, auc_val, acc_val = validate(loader_val, model, criterion, evaluator_val, args)
    ...
    loc = 'cuda:{}'.format(args.gpu)
    checkpoint = torch.load(f'results/{args.identifier}.pth.tar', map_location=loc)
    model.load_state_dict(checkpoint['state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer'])
...
def validate(val_loader, model, criterion, evaluator, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True) #[1003, 3, 28, 28])
            # if torch.cuda.is_available():
                target = target.squeeze().long().cuda(args.gpu, non_blocking=True) #1003
